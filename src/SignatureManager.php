...
class SignatureManager
{
    ...
    public function __construct(array $config)
    {
        ...
        if ($typeId) {
            $this->setClient(Arr::getValue($config, 'client'));
            $this->setServer(Arr::getValue($config, 'server'));
    ...
    protected function signParameters(array $params = [])
    {
        ...
        if ($params) {
            ...
            foreach ($params as $key => $value) {
                $string .= urlencode($key) .'='. urlencode(is_array($value) ? json_encode($value) : $value) . $this->getConnector();
            }
    ...
    protected function loadConfig()
    {
        if ($this->getIsServer()) {
            ...
            $this->headerVersion = Arr::getValue($server, 'headerVersion');
            ...
            $this->headerVersion = Arr::getValue($client, 'headerVersion');
